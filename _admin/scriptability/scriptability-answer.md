...
- [export](#answer-export) the object(s), edit the object(s) by modifying its [ThoughtSpot Scripting Language]({{ site.baseurl }}/admin/scriptability/tsl-answer.html) (`TSL`) representation, and [import](#answer-update) the updated file(s) to modify the existing object *or*
- edit the object(s) using the [in-app `TSL` editor](#edit-tsl) and publish the updated file(s).
2. **Migrate an existing object from one cluster to a new cluster**: [export](#answer-export) the object(s) and [import](#answer-migrate) the updated file(s) to the new cluster.
3. **Edit and migrate an existing object from one cluster to a new cluster**: You can either
- [export](#answer-export) the object(s), edit the object(s) by modifying its [ThoughtSpot Scripting Language]({{ site.baseurl }}/admin/scriptability/tsl-answer.html) (`TSL`) representation, and [import](#answer-migrate) the updated file(s) to the new cluster *or*
- edit the object(s) using the [in-app `TSL` editor](#edit-tsl), publish the updated file(s), [export](#answer-export) the object(s), and [import](#answer-migrate) the updated file(s) to the new cluster. Note that this workflow changes the object(s) in both clusters.
...
You can edit the `TSL` file in one of two ways. You can [export](#answer-export) the Answer(s) and edit the file(s) in any text editor, before you import it. Or, you can use the [in-app `TSL` editor](#tsl-editor) to edit, validate, and publish the Answer(s). Refer to [Answer TSL specification]({{ site.baseurl }}/admin/scriptability/tsl-answer.html) for information on syntax in the TSL files.

{: id="tsl-editor"}
## Edit, validate, and publish Answers using the TSL editor
You can access the TSL editor from the Answer list page, or from the Answer itself. To edit and update multiple objects using the TSL editor, access it from the object list page.

To use the TSL editor, follow these steps:

1. Navigate to the **Answers** page from the top navigation bar.

2. Click the name of the Answer you want to edit, or select multiple Answers by clicking on the checkboxes that appear when you hover over an Answer name.

3. From the Answer list page, select the **Edit TSL** button that appears when you select an Answer or Answers. From the Answer itself, select the ellipsis ![more options menu]({{ site.baseurl }}/images/icon-ellipses.png){: .inline} (more options) menu in the upper-right side of the screen, and select **Edit TSL**.

4. The TSL editor opens. Edit the TSL file(s), using the syntax specified in [Answer TSL specification]({{ site.baseurl }}/admin/scriptability/tsl-answer.html).
...
8. The system displays a **Publish status** dialog box. You can select **Open Answer** to open the Answer you just published in a new tab, or click **Close** to return to the TSL editor.

    ![Open the Answer or return to the TSL editor]({{ site.baseurl }}/images/scriptability-tsl-editor-publish-status.png "Open the Answer or return to the TSL editor")

{: id="answer-update"}
## Update an Answer
You can overwrite an existing Answer by downloading the `TSL` file, making any necessary changes, and then re-uploading the `TSL` file.

You can also update an object using the [TSL editor](#tsl-editor).

To update an existing Answer by downloading the TSL file and modifying it, follow these steps. In this case, we are updating a single Answer. You can update multiple objects at once by uploading them in .zip file format.

1. [Export the Answer](#answer-export) you want to update, as in steps 1 to 5 of the **Export Answer** section above.
...
9. If you uploaded a `.zip` file with multiple Answers, you can unselect any files in the `.zip` file you do not want to upload.
...
11. The **Import Status** screen displays the status of the Answers you imported. You can open the Answer(s) that you imported, or click **Done** to return to the main object page.
...
3. Click **Answers** on the top navigation bar.

4. Select **Import TSL** in the upper right-hand corner of the screen.
