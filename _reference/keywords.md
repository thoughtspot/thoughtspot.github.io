...
Starting with release 6.3, when you use more than one `begins with` or `ends with` keyword in a search or formula, ThoughtSpot returns your results using an `OR` condition, instead of an `AND` condition. When you use a combination of `begins with` and `ends with`, ThoughtSpot still returns your results using an `AND` condition.
