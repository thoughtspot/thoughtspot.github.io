...
* `tscli fileserver download-release [-h] [--user` *`USER`*`] [--password` *`PASSWORD`*`]` *`release`* Downloads the specified release file, its checksum and verify integrity of release bundle. You'll need to specify the exact release number (e.g. 5.1.3). Before using this command for the first time, you need to set up the file server connection using `tscli fileserver configure`. You will then work with a member of ThoughtSpot Support team since a privileged `--user` (and corresponding `--password`) need to be specified to download releases.
