# Note that .sh scripts work only on Mac. If you're on Windows, install Git Bash and use that as your client.
    ...
    for book in $(echo "$jsonfile" | jq -r '.books[] | @base64'); do
        ...
        sed -i "" "s/THEVERSION/${theVersion}/g" ${DIR}/pdfconfigs/config_mydoc_pdf-aws.yml
        ...
        sed -i "" "s/THETITLE/${theTitle}/g" ${DIR}/pdfconfigs/config_mydoc_pdf-aws.yml

        for code in $(echo $(_jq '.codes') | jq -r '.[]'); do
          theCode=${code}
          sed -i "" "s/${code}/pdf/g" ${DIR}/_data/sidebars/mydoc_sidebar-aws.yml
        done
        ...
        bundle exec jekyll serve --detach --config _config.yml,pdfconfigs/config_mydoc_pdf-aws.yml;
        echo "done";
        ...
        git checkout -- ${DIR}/pdfconfigs/config_mydoc_pdf-aws.yml
        git checkout -- ${DIR}/_data/sidebars/mydoc_sidebar-aws.yml
    done
